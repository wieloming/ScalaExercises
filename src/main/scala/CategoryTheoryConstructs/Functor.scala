...
trait Functor[F[_]] extends GenericFunctor[Function, F] {
  self =>
...
//    def COMPOSE[G[_]](implicit G: Functor[G]): Functor[({type L[a] = F[G[a]]})#L] =
//      new Functor[({type L[a] = F[G[a]]})#L] {
//        override def MAP[A, B](as: F[G[A]])(f: (A) => B): F[G[B]] = {
//          MAP(as)(f)
//        }
//      }
}
